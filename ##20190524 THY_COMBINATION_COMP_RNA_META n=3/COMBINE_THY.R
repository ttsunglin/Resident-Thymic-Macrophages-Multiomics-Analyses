...
paste("System had paused and would resume in 3 minutes.")
Sys.sleep(120)
paste("resume in 1 minute. Do not proceed without mis-match corrected")
Sys.sleep(60)
paste("Program resuming.")
 ###########################
 #   Caution: pause here   #
...
pc_my_gene <- read.csv("PC_my_gene.csv")
pc_my_gene <- data.frame(pc_my_gene[,-1]) 
colnames(pc_my_gene) <- "EZ_ID"
check_repeat_gene <- merge(THY_KEGGID, pc_my_gene, by = "EZ_ID")
...
match_a <- match_a[,-(25:37)]
row.names(match_a) <- c(1:nrow(match_a))
match_a2 <- match_a
match_a2$`keggid` <- apply(match_a2$keggid, 1, as.character)
colnames(match_a)[26] <- "keggid"

#showing all detected features that match to given pathway
MATCHCOMPTOPATH2 <- function(x){
  
  comp_path <- data.frame(PATHCOMPINFO2(x))
  comp_path$keggid <- rownames(comp_path)
  colnames(comp_path) <- c("names","keggid")
  out <- suppressWarnings(merge(match_a2, comp_path, by ="keggid"))
  out$tn1mean <- apply(out[,8:10], 1, mean)
  out$tn2mean <- apply(out[,11:13], 1, mean)
  out$tn3mean <- apply(out[,14:16], 1, mean)
  out$pn1mean <- apply(out[,17:19], 1, mean)
  out$pn2mean <- apply(out[,20:22], 1, mean)
  out$pn3mean <- apply(out[,23:25], 1, mean)
  
  if(nrow(out) == 0){
    warning(paste("path comp not matched."))
  }else{
    out
  }
}
...
HEATMAPCOMP <- function(x){
  data_comp <- MATCHCOMPTOPATH2(x)
  data_comp <- data_comp[,c(1, 27, 28:33)]
  data_comp <- distinct(data_comp)
  data_ext <- data_comp[,3:8]
  data_ext <- distinct(data_ext)
  #row.names(data_ext) <- data_comp$`names`
  c <- data.matrix(data_ext)
  plot <- heatmap(c,
                  Colv = NA,
                  scale = "row",
  ...
  plot
  #error might occur when different m/z matched to same keggid
}

#----------------------*all compound heatmap---------
match_a3 <- match_a2
match_a3$tn1mean <- apply(match_a3[,7:9], 1, mean)
match_a3$tn2mean <- apply(match_a3[,10:12], 1, mean)
match_a3$tn3mean <- apply(match_a3[,13:15], 1, mean)
match_a3$pn1mean <- apply(match_a3[,16:18], 1, mean)
match_a3$pn2mean <- apply(match_a3[,19:21], 1, mean)
match_a3$pn3mean <- apply(match_a3[,22:24], 1, mean)

match_a4 <- distinct(match_a3[,-(2:26)])
match_a4$allmean <- apply(match_a4[,2:7], 1, mean)
match_a5 <- match_a4[(which(match_a4$allmean >= 100)),] 

match_a6 <- match_a5[,-c(1,8)]
row.names(match_a6) <- match_a5$Compound
match_mx <- data.matrix(match_a6)
write.table(match_a6, file = "output/compound_mx_heatmap.csv", sep = ",")
plot <- heatmap(match_mx,
                Colv = NA,
                scale = "row",
                col = rev(brewer.pal(11,"RdBu")))

#----------------------*my compound heatmap---------
my_compound_df <- kegg_id_df_thy2
colnames(my_compound_df) <- "keggid"
my_compound_mx <- merge(match_a3, my_compound_df)
my_compound_mx2 <- distinct(my_compound_mx[,c(2,27:32)])
my_compound_mx3 <- my_compound_mx2[,-1]
row.names(my_compound_mx3) <- my_compound_mx2$Compound
write.table(my_compound_mx3, file = "output/my_met_compound_mx_heatmap.csv", sep = ",")
my_compound_mx4 <- as.matrix(my_compound_mx3)
plot <- heatmap(my_compound_mx4,
                Colv = NA,
                scale = "row",
                col = rev(brewer.pal(11,"RdBu")))
#----------------------*Run-----------------------------
